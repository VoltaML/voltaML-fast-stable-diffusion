...
def get_freq_filter(shape, device, params: dict):
    ...
    if params["method"] == "gaussian":
        return gaussian_low_pass_filter(
            shape=shape, d_s=params["d_s"], d_t=params["d_t"]
        ).to(device)
        ...
        return box_low_pass_filter(
            shape=shape, d_s=params["d_s"], d_t=params["d_t"]
        ).to(device)
    elif params["method"] == "butterworth":
        return butterworth_low_pass_filter(
            shape=shape, n=params["n"], d_s=params["d_s"], d_t=params["d_t"]
        ).to(device)
    else:
