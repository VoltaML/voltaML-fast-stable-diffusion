...
class CrossAttnStoreProcessor:
    ...
    def __init__(self):
        ...
        self.use_sdpa = any(
            [x == config.api.attention_processor for x in ["sdpa", "xformers"]]
        )
    ...
    def __call__(
        ...
        if not self.use_sdpa:
            ...
            self.attention_map = (
                attention_probs.reshape(batch_size, -1, *attention_probs.shape[1:])
                .mean(1)
                .sum(1)
            )
            ...
            dummy_value_eye = torch.eye(
                value.shape[2], device=value.device, dtype=value.dtype
            ).expand(batch_size, attn.heads, -1, -1)
            self.attention_map = (
