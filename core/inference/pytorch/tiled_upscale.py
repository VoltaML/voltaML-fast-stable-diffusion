...
def clamp_rect(rect: List[int], min_: List[int], max_: List[int]):
    return (
        clamp(rect[0], min_[0], max_[0]),
        clamp(rect[1], min_[1], max_[1]),
        clamp(rect[2], min_[0], max_[0]),
        clamp(rect[3], min_[1], max_[1]),
    )


def add_overlap_rect(rect: List[int], overlap: int, image_size: List[int]):
    rect = list(rect)
    rect[0] -= overlap
    rect[1] -= overlap
    rect[2] += overlap
    rect[3] += overlap
    rect = clamp_rect(rect, [0, 0], [image_size[0], image_size[1]])  # type: ignore
    return rect
...
class StableDiffusionTiledUpscalePipeline(StableDiffusionUpscalePipeline):
    ...
    def _process_tile(
        ...
        **kwargs,
        ...
        crop_rect_with_overlap = add_overlap_rect(crop_rect, tile_border, image.size)  # type: ignore
    ...
    def __call__(  # pylint: disable=signature-differs
        ...
        print(
            f"total tiles: {total_tile_count}, tcx: {tcx}, tcy: {tcy}, tile_size: {tile_size}, image_size: {image.size}"
        )
        current_count = 0
        for y in range(tcy):
            for x in range(tcx):
                ...
                current_count += 1
                print(f"progress: {current_count / total_tile_count:.4f}")
        return final_image
