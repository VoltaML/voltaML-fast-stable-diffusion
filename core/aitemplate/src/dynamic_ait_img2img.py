...
class StableDiffusionDynamicAITPipeline(StableDiffusionPipeline):
    def __init__(  # pylint: disable=super-init-not-called
        ...
        # CLIP
        self.clip_ait_exe = clip_ait_exe or self.init_ait_module(
            model_name="CLIPTextModel", workdir=directory
        )
        logger.debug("Loading PyTorch CLIP")
        self.clip_pt = text_encoder
        ...
        # UNet
        self.unet_ait_exe = unet_ait_exe or self.init_ait_module(
            model_name="UNet2DConditionModel", workdir=directory
        )
        logger.debug("Loading PyTorch UNet")
        self.unet_pt = unet
        ...
        self.vae_ait_exe = vae_ait_exe or self.init_ait_module(
            model_name="AutoencoderKL", workdir=directory
        )
        logger.debug("Loading PyTorch VAE")
        self.vae_pt = vae
        in_channels = 3
        out_channels = 3
        down_block_types = [
        ...
        ait_vae = ait_AutoencoderKL(
            ...
            block_out_channels=block_out_channels,  # type: ignore
        ...
        self.scheduler: LMSDiscreteScheduler = scheduler  # type: ignore
        self.batch = 1
    ...
    def __call__(
        ...
        for i, t in enumerate(self.progress_bar(self.scheduler.timesteps[t_start:])):
            ...
            noise_pred = self.unet_inference(
                ...
                height=init_image.shape[2],  # type: ignore
                width=init_image.shape[3],  # type: ignore
            )
        ...
        image = self.vae_inference(latents, init_image.shape[2], init_image.shape[3])  # type: ignore
        # pytorch equivalent
