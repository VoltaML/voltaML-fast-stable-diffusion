...
from ..modeling_dynamic.controlnet_unet_2d_condition import (
...
def compile_controlnet(
    ...
    latent_model_input_ait = Tensor(
        [batch_size, height // 8, width // 8, 4], name="input0", is_input=True  # type: ignore
    )
    timesteps_ait = Tensor([batch_size], name="input1", is_input=True)  # type: ignore
    text_embeddings_pt_ait = Tensor(
        [clip_batch_size, embedding_size, hidden_dim], name="input2", is_input=True  # type: ignore
    )
    controlnet_condition_ait = Tensor(
        [batch_size, height, width, 3], name="input3", is_input=True  # type: ignore
    )
