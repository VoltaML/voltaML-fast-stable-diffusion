...
class ControlNetConditioningEmbedding(nn.Module):
    ...
    def forward(self, conditioning: Tensor) -> Tensor:
        pad = ops.nhwc3to4()
        conditioning = pad(conditioning)  # type: ignore
...
class ControlNetModel(nn.Module):
    ...
    def __init__(
        ...
        conditioning_embedding_out_channels: Optional[Tuple[int, int, int, int]] = (
            16,
            32,
            96,
            256,
        ...
        self.mid_block = UNetMidBlock2DCrossAttn(
            ...
            resnet_groups=norm_num_groups,  # type: ignore
            use_linear_projection=use_linear_projection,
        )

    def get_shape(self, sample):
        return [i._attrs["int_var"]._attrs["values"][0] for i in ops.size()(sample)]  # type: ignore
