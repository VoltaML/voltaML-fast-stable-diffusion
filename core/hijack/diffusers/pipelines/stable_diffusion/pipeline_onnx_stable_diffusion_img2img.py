...
class OnnxStableDiffusionImg2ImgPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        text_embeddings = self.text_encoder(input_ids=text_input_ids.astype(np.int32))[
            0
        ]
        ...
        if do_classifier_free_guidance:
            ...
            uncond_embeddings = self.text_encoder(
                input_ids=uncond_input_ids.astype(np.int32)
            )[0]
        ...
        if isinstance(prompt, str):
            prompt = [prompt]
        if (
            len(prompt) > init_latents.shape[0]
            and len(prompt) % init_latents.shape[0] == 0
        ):
