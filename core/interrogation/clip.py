...
from transformers import AutoProcessor, AutoModelForCausalLM, BlipForConditionalGeneration, Blip2ForConditionalGeneration, PreTrainedModel
...
class CLIPInterrogator(InterrogationModel):
    "internal"
    def __init__(self, device: str = "cuda", use_fp32: bool = False, autoload: bool = False):
        ...
        self.dtype: torch.dtype = torch.float32 if use_fp32 else (torch.bfloat16 if is_cpu(device) else torch.float16)
    ...
    def _interrogate_positive(self, image: Image.Image, max_flavors: int = 32, caption: Optional[str] = None) -> str:
        if caption is None:
            inputs = self.caption_processor(images=image, return_tensors="pt").to(self.device, dtype=self.dtype) # type: ignore
            tokens = self.caption_model.generate(**inputs, max_new_tokens=config.interrogator.caption_max_length)
    ...
    def _interrogate_negative(self, image: Image.Image, max_flavors: int = 32) -> str:
        def _chain(
            ...
            if not best_prompt:
                ...
                best_prompt = self._rank_top(image_features, [f for f in phrases], reverse=reverse)
            ...
            for idx in tqdm(range(max_count), desc=descriptor):
                best = self._rank_top(image_features, [f"{curr_prompt, {f}}" for f in phrases], reverse=reverse)
            ...
            return best_prompt
        image_features = self._image_to_features(image)
        flaves = [x for x in self.labels if x.descriptor == "flavors"][0].rank(image_features, config.interrogator.flavor_intermediate_count, reverse=True)
        flaves = flaves + [x for x in self.labels if x.descriptor == "negative"][0].labels
        return _chain(image_features, flaves, max_count=max_flavors, reverse=True, descriptor="Negative chain")


    def _rank_top(self, image_features: torch.Tensor, text_array: List[str], reverse: bool = False) -> str:
        text_tokens = self.tokenize([text for text in text_array]).to(self.device, dtype=self.dtype)
        with torch.no_grad(), torch.autocast(device_type="cpu" if is_cpu(self.device) else "cuda", dtype=self.dtype): # type: ignore
            text_features = self.clip_model.encode_text(text_tokens) # type: ignore
            text_features /= text_features.norm(dim=-1, keepdim=True)
            similarity = text_features @ image_features.T
    ...
    def load(self):
        ...
        match config.interrogator.caption_model.split("/")[1].split("-")[0].lower():
            case "git":
                ...
                self.caption_model = AutoModelForCausalLM.from_pretrained(config.interrogator.caption_model, torch_dtype=self.dtype)
            case "blip2":
        ...
        if config.interrogator.offload_captioner:
            if is_accelerate_available() and self.device != "cpu":
                ...
                logger.warning("Accelerate is not available. Check your installation. Skipping offload.")
        else:
            logger.info("Loaded BLIP interrogator %s on device %s. DType: %s", config.interrogator.caption_model, self.device, str(self.dtype))
        ...
        self.clip_model, _, self.clip_preprocess = open_clip.create_model_and_transforms(
            ...
            precision="fp16" if self.dtype == torch.float16 and not is_cpu(self.device) else "fp32",
            device=self.device,
            jit=False,
            cache_dir=str(self.cache_path)
        )
        self.clip_model.eval()  # type: ignore
        self.tokenize = open_clip.get_tokenizer(config.interrogator.visualizer_model.split("/")[0])
        self.file_folder = get_full_model_path("tokens", model_folder="clip", force=True)
        ...
        for file in [x for x in os.listdir(str(self.file_folder)) if Path(x).stem != "artists"]:
...
class LabelTable():
    ...
    def __init__(self, labels: List[str], descriptor: str, interrogator):
        ...
        sanitized_name = config.interrogator.caption_model.replace("/", "_").replace("@", "_")
        ...
        if len(self.labels) != len(self.embeds):
            ...
            for chunk in tqdm(chunks, desc=f"Preprocessing {descriptor}" if descriptor else None):
            ...
            if descriptor and self.cache_path:
                ...
                cache_file = self.cache_path.joinpath(f"{sanitized_name}_{descriptor}.safetensors")
                tensors = {
    ...
    def _load_cached(self, descriptor: str, hash: str, sanitized_name: str) -> bool: # pylint: disable=redefined-builtin
        cached_safetensors = self.cache_path.joinpath(f"{sanitized_name}_{descriptor}.safetensors")
    ...
    def _rank(self, image_features: torch.Tensor, text_embeds: torch.Tensor, top_count: int=1, reverse: bool=False) -> str:
        ...
        text_embeds = torch.stack([torch.from_numpy(t) for t in text_embeds]).to(self.device, dtype=self.dtype)
        with torch.no_grad(), torch.autocast(device_type="cpu" if is_cpu(self.device) else "cuda", dtype=self.dtype): # type: ignore
