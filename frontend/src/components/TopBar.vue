                  ...
                  <NCard :title="lora_title">
                    <div v-if="global.state.selected_model !== null">
                      ...
                      <div style="
                          ...
                          justify-content: space-between;
                          border-bottom: 1px solid rgb(66, 66, 71);
                        " v-for="lora in loraModels" v-bind:key="lora.path">
                        <p>{{ lora.name }}</p>
                        <div style="display: inline-flex">
                          <NButton type="error" ghost @click="unloadLoRA(lora)" :disabled="global.state.selected_model === undefined
                            " v-if="global.state.selected_model?.loras.includes(
    lora.path
  )
    ">Unload</NButton>
                          <NButton type="success" ghost @click="loadLoRA(lora)" :disabled="global.state.selected_model === undefined
                            " :loading="lora.state === 'loading'" v-else>Load</NButton>
                        </div>
                      </div>
                    </div>
                    <div v-else>
...
async function unloadLoRA(lora: ModelEntry) {
  if (global.state.selected_model) {
    try {
      await fetch(`${serverUrl}/api/models/unload-lora`, {
        method: "POST",
        headers: {
      ...
      global.state.selected_model.loras.splice(global.state.selected_model.loras.indexOf(lora.path), 1);
    } catch (e) {
      console.error(e);
    }
  } else {
    ...
    try {
      await fetch(`${serverUrl}/api/models/load-lora`, {
        ...
        body: JSON.stringify({
          model: global.state.selected_model.name,
          lora: lora.path,
          weight: conf.data.settings.api.lora_weight,
        }),
      });
...
const lora_title = computed(() => {
  return `LoRA (${global.state.selected_model
